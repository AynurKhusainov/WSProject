...
public class Profile extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater,
        ...
        for (int i = 0; i < name.length; i++) {
            ...
            ProfileModel profileModel = new ProfileModel(name[i]);
            profileModelArrayList.add(profileModel);
        }
        ProfileAdapter listAdapter = new ProfileAdapter(getContext(), profileModelArrayList);
        binding.profileList.setAdapter(listAdapter);
        changeEmail();
        profile_third();
        return v;
    }
    private void profile_third() {

        ImageButton f=v.findViewById(R.id.information);
